...
class TransportationCosts(models.Model):
    _name = 'transportation_costs'
    _description = '交通费用'
    ...
    def confirmation_button(self):
        ...
        action = {
            'name': name,
            'view_mode': 'form',
            'res_model': 'transportation_costs',
            'view_id': self.env.ref('fsn_travel.transportation_costs_form').id,
    ...
    def write(self, vals):
        ...
        res = super(TransportationCosts, self).write(vals)
    ...
    def unlink(self):
        ...
        res = super(TransportationCosts, self).unlink()
