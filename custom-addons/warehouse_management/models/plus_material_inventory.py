...
class PlusMaterialInventory(models.Model):
    _name = 'plus_material_inventory'
    _description = '仓库布料单(库存)'
    ...
    plus_material_enter_id = fields.One2many("plus_material_enter", "plus_material_inventory_id", string="入库明细")

    outbound_ids = fields.One2many("plus_material_outbound", "plus_material_inventory_id", string="出库明细")


    # 计算库存数量
    @api.depends('plus_material_enter_id', 'outbound_ids', 'plus_material_enter_id.amount', 'outbound_ids.amount')
    def set_amount(self):
        for record in self:

            record.amount = Decimal(str(sum(Decimal(str(i.amount)) for i in record.plus_material_enter_id))) - Decimal(str(sum(Decimal(str(i.amount)) for i in record.outbound_ids)))
