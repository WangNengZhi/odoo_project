...
class SpareFabricEnter(models.Model):
    _name = 'spare_fabric_enter'
    _description = '备用面料入库'
    ...
    @api.depends('spare_fabric_enter_line_ids', 'spare_fabric_enter_line_ids.amount', 'unit_price')
    def set_money_sum(self):
        for record in self:
            record.amount = sum(record.spare_fabric_enter_line_ids.mapped("amount"))
    ...
    spare_fabric_enter_line_ids = fields.One2many("spare_fabric_enter_line", "spare_fabric_enter_id", string="备用面料入库明细")


    spare_fabric_inventory_id = fields.Many2one("spare_fabric_inventory", string="备用面料库存")


    @api.constrains('spare_fabric_enter_line_ids', 'spare_fabric_enter_line_ids.batch_number')
    def _check_unique(self):
        for record in self:
            tem_list = record.spare_fabric_enter_line_ids.mapped("batch_number")
    ...
    def affirm_enter(self):
        for record in self:
            ...
            inventory_obj = self.env["spare_fabric_inventory"].sudo().search([("material_coding", "=", record.material_coding.id)])
            if not inventory_obj:
                inventory_obj = self.env["spare_fabric_inventory"].sudo().create({"material_coding": record.material_coding.id})
            record.spare_fabric_inventory_id = inventory_obj.id
            record.state = "已入库"
    ...
    def back_button(self):
        for record in self:
            ...
            record.state = "草稿"
            record.spare_fabric_inventory_id = False
            record.set_procurement_complete()
    ...
    def write(self, vals):
        ...
        res = super(SpareFabricEnter, self).write(vals)
        return res
    ...
    def unlink(self):
        ...
        res = super(SpareFabricEnter, self).unlink()
        return res


class SpareFabricEnterLine(models.Model):
    _name = 'spare_fabric_enter_line'
    _description = '备用面料入库明细'

    spare_fabric_enter_id = fields.Many2one("spare_fabric_enter", string="备用面料入库", ondelete="cascade")
    batch_number = fields.Integer(string="批次",  required=True)
    amount = fields.Float(string="数量")



class SpareFabricInventory(models.Model):
    _name = 'spare_fabric_inventory'
    _description = '备用面料库存'
    _rec_name = 'material_coding'


    material_coding = fields.Many2one("plus_material_list", string="面料编号")
    order_id = fields.Many2one('sale_pro.sale_pro', string='订单号', related="material_coding.order_id", store=True)
    style_number = fields.Many2one('ib.detail', string='款号', related="material_coding.style_number", store=True)
    ...
    spare_fabric_enter_ids = fields.One2many("spare_fabric_enter", "spare_fabric_inventory_id", string="备用面料入库")

    plus_material_enter_ids = fields.One2many("plus_material_enter", "spare_fabric_inventory_id", string="面料入库")


    # 计算库存数量
    @api.depends('spare_fabric_enter_ids', 'spare_fabric_enter_ids.amount', 'spare_fabric_enter_ids.state',\
                'plus_material_enter_ids', 'plus_material_enter_ids.amount', 'plus_material_enter_ids.state')
    def set_amount(self):
        for record in self:

            enter_amount = Decimal(str(sum(Decimal(str(i.amount)) for i in record.spare_fabric_enter_ids if i.state == "已入库")))
            out_amount = Decimal(str(sum(Decimal(str(i.amount)) for i in record.plus_material_enter_ids if i.state == "已入库")))
    ...
    def conversion_materials_stored(self):
        ...
        action = {
            'name': "转为用料入库",
            'view_mode': 'form',
            'res_model': 'spare_fabric_inventory_wizard',
            'view_id': self.env.ref('warehouse_management.spare_fabric_inventory_wizard_form').id,
            'type': 'ir.actions.act_window',
            'target': 'new',
            'context': {
                'default_spare_fabric_inventory_id': self.id,
            }
        }
        return action
...
class PlusMaterialEnter(models.Model):
    ...
    spare_fabric_inventory_id = fields.Many2one("spare_fabric_inventory", string="备用面料库存", ondelete="restrict")
    spare_fabric_out_state = fields.Selection([('草稿', '草稿'),('已出库', '已出库')], string="备用出库状态", compute="set_spare_fabric_out_state", store=True)
    @api.depends("state")
    def set_spare_fabric_out_state(self):
        for record in self:
            if record.state == "草稿":
                record.spare_fabric_out_state = "草稿"
            
            if record.state == "已入库":
                record.spare_fabric_out_state = "已出库"


class SpareFabricInventoryWizard(models.TransientModel):
    _name = 'spare_fabric_inventory_wizard'
    _description = '备用面料库存向导'
    ...
    def set_odd_numbers(self):
        spare_fabric_inventory_obj = self.env['spare_fabric_inventory'].sudo().browse(self.env.context.get("default_spare_fabric_inventory_id"))
        return [(i.odd_numbers, i.odd_numbers) for i in spare_fabric_inventory_obj.spare_fabric_enter_ids]
    ...
    def confirm_conversion_materials_stored(self):
        ...
        self.env['plus_material_enter'].sudo().create({
            "date": self.date,
            "odd_numbers": self.odd_numbers,
            "material_coding": self.spare_fabric_inventory_id.material_coding.id,
            "order_id": self.order_id.id,
            "style_number": self.style_number.id,
            "spare_fabric_inventory_id": self.spare_fabric_inventory_id.id,
            "consigner": self.responsible_person.name,
            "select_consignee":  self.responsible_person.id,
            "plus_material_enter_line_ids": [(0, 0, {'batch_number': 1, 'amount': self.amount})]
        })