...
class WarehouseBomOutbound(models.Model):
    _name = 'warehouse_bom_outbound'
    _description = '仓库物料单(出库)'
    ...
    inventory = fields.Many2one("warehouse_bom_inventory", string="库存", required=True)
    material_coding = fields.Many2one("material_list", string="物料编码", related="inventory.material_coding", store=True)
    ...
    consignee_id = fields.Many2one('hr.employee', string="收货人", required=True)
    ...
    def affirm_outbound(self):
        for record in self:
            ...
            if self.amount > self.inventory.amount:
                ...
                record.warehouse_bom_inventory_id = record.inventory.id
                record.state = "已出库"
    ...
    def write(self, vals):
        ...
        if self.state == "已出库":
            ...
            if ("state" in vals or "surface_accessories_loss_id" in vals) and len(vals) == 1:
                ...
                raise ValidationError(f"{self.odd_numbers}已经出库，不可修改！。")

        res = super(WarehouseBomOutbound, self).write(vals)
    ...
    def create(self, vals):
        ...
        instance = super(WarehouseBomOutbound, self).create(vals)
    ...
    def unlink(self):
        ...
        return super(WarehouseBomOutbound, self).unlink()
