    ...
    def generate_salary_sheet(self):
        for record in self:
            ...
            if "未确认" in state_list or "有更改" in state_list:
                ...
                fsn_salary_sheet_obj = self.env["fsn_salary_sheet"].sudo().search([("month", "=", record.date), ("employee_id", "=", record.name.id)])

                if fsn_salary_sheet_obj:
                    fsn_salary_sheet_obj.sudo().unlink()

                self.env["fsn_salary_sheet"].sudo().create({
...
class FsnSalarySheet(models.Model):
    _name = 'fsn_salary_sheet'
    _description = 'fsn_工资表'
    ...
    salary_payable = fields.Float(string="应发工资")
    ...
    paid_wages = fields.Float(string="实发工资")
